...
export default function TrackManager() {
  ...
  async function addToQueue(track) {
    setQueueing(track.id);
    try {
      await apiCall('POST', '/api/admin/queue', {
        env: track.env,
        track: track.track,
        race: '',
      }, 'Added to queue');
      ...
      <div style={styles.rightPanel}>
        {!selected ? (
          ...
          <>
            <TrackDetailHeader
              track={selected}
              onQueue={() => addToQueue(selected)}
              queueing={queueing === selected.id}
            />
...
function TrackListItem({ track, selected, onClick, onQueue, queueing }) {
...
function TrackDetailHeader({ track, onQueue, queueing }) {
  const imgUrl = imageProxy(track.steam_preview_url);
  return (
    <div style={styles.detailHeader}>
      ...
      <button
        ...
        style={styles.detailQueueBtn}
        onClick={onQueue}
        disabled={queueing}
      >
        {queueing ? 'Queuing...' : 'Add to Queue'}
      </button>
    </div>
  );
}
        ...
        <div style={styles.tagCloud}>
          ...
          {systemTags.map(t => (
            <span key={t.id} style={styles.tagChip}>
              {t.name}
              <button style={styles.tagRemove} onClick={() => removeSystemTag(t.id)} title="Remove">&times;</button>
            </span>
          ))}
        </div>
        {available.length > 0 && (
...
const styles = {
  ...
  detailQueueBtn: { flexShrink: 0, marginLeft: 'auto' },
